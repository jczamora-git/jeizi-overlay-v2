...
router.get("/", async (req, res) => {
  try {
    ...
    res.status(500).json({
      ...
      client: process.env.DEBUG_API_ERRORS === "true" ? db.client : undefined,
    });
  }
});
  ...
  try {
    const sql =
      db.client === "postgres"
        ...
        : "SELECT * FROM matches ORDER BY queue_order ASC, id ASC";
    ...
    console.error("[matches test-basic error]", error);

    res.status(500).json({
      ok: false,
      message: "Failed to test matches table",
      client: db.client,
      details: process.env.DEBUG_API_ERRORS === "true" ? error.message : undefined,
      code: process.env.DEBUG_API_ERRORS === "true" ? error.code : undefined,
    });
